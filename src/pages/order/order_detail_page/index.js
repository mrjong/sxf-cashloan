...
export default class order_detail_page extends PureComponent {
	...
	handleClickConfirm = () => {
		...
		let repayStswStr = '';
		if (isPayAll) {
			if (billDesc.billPerdStsw.indexOf('1') > -1) {
				// 只适用里面有一个1的情况
				repayStswStr =
					billDesc.billPerdStsw.split('1')[0] + '1' + billDesc.billPerdStsw.split('1')[1].replace(/0/g, '1');
			} else {
				return;
			}
			sendParams = {
	...
	repay = () => {
		const { billDesc, isPayAll, isNewsContract, repayParams, isSettle, totalAmt, payType } = this.state;
