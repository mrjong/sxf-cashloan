      ...
      .then(
        res => {
          if (res.msgCode === 'PTM3021') {
            ...
            buriedPointEvent(membership.confirmBuyPre, {
              entry: query && query.entryFrom ? query.entryFrom : 'mine',
              is_success: true,
            });
  ...
  render() {
    ...
    return (
      <div className={styles.membership_card_page}>
        {this.state.status && this.state.cardState === '0'
          ? data &&
            data.map((item, index) => {
              return (
                <div
                ...
                >
                  ...
                  <div className={styles.cardTitle}>
                    <span className={styles.left}>{(item.memPrdDes && item.memPrdDes.split('|')[0]) || '--'}</span>
                    <span className={styles.right}>{(item.memPrdDes && item.memPrdDes.split('|')[1]) || '--'}</span>
                  </div>
                  <div className={styles.cardTitle}>
                    有效期
                    {item.menTerm || '--'}
          ...
          : null}
        {this.state.status && this.state.cardState === '1'
          ? cardInfo &&
            cardInfo.memList &&
            cardInfo.memList.map((item, index) => {
              return (
                <div
                  key={index}
                  onClick={() => {
                ...
                >
                  ...
                  <div className={styles.cardTitle}>
                    有效期至
                    {item.cardExpDt && dayjs(item.cardExpDt).format('YYYY/MM/DD')}
                  </div>
                </div>
              );
            })
        ...
        {this.state.status && cardState === '0' ? (
          <div className={styles.btn}>
            <ButtonCustom onClick={this.buy} className={styles.sureBtn}>
              确认购买
            </ButtonCustom>
          </div>
        ) : null}
