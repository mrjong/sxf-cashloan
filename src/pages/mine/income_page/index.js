...
export default class income_page extends PureComponent {
			...
			.then((res) => {
				...
				if (res.msgCode === 'PTM0000') {
					...
					for (let i = res.data.usrRedCoupons.length - 1; i >= 0; i--) {
						...
						dataArr.push(res.data.usrRedCoupons[i]);
					}
					return dataArr;
				}
				return [];
	...
	render() {
		...
		const row = (rowData, sectionID, rowID) => {
			...
			return (
				// 状态，0：失效，1：正常，2：提现中，3：已提现,4:冻结 5:失效
				<div className={style.incomeBox}>
					<h2 className={style.incomeTit}>{obj.groupNm}</h2>
					<div
						className={
							obj.sts === '5'
								? `${style.incomeCont} ${style.incomeContExpired}`
								: obj.sts === '3'
								? `${style.incomeCont} ${style.incomeContSucc}`
								: style.incomeCont
						}
					>
						<img className={style.redBag} src={redBagIco} />
						<div className={style.incomeContent}>
							<p className={style.incomeMoney}>
								¥<span>{obj.couponAmt}</span>
							</p>
