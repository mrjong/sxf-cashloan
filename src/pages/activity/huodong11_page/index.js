...
@fetch.inject()
@createForm()
export default class huodong11_page extends PureComponent {
	constructor(props) {
		super(props);
		this.state = {
			timers: '获取验证码',
	...
	getStatus = (id, type) => {
		// 返现上一次刷新的状态
		if (localStorage.getItem(`${id}img5`)) {
			this.setState({
				img5: localStorage.getItem(`${id}img5`)
			});
		}
		if (localStorage.getItem(`${id}img7`)) {
			this.setState({
				img7: localStorage.getItem(`${id}img7`)
			});
		}
		if (localStorage.getItem(`${id}img10`)) {
			this.setState({
				img10: localStorage.getItem(`${id}img10`)
			});
		}
		if (localStorage.getItem(`${id}img15`)) {
			this.setState({
				img15: localStorage.getItem(`${id}img15`)
			});
		}
		if (type) {
			this.typeUp(id, type);
	...
	imgClick = (type) => {
		const token = Cookie.get('fin-v-card-token');
		if (!token) {
			buriedPointEvent(activity.login_open);
			this.setState({
				login_box: true,
				type
			});
		...
		switch (type) {
			case '5':
				...
				setTimeout(() => {
					...
					localStorage.setItem(`${id}img5`, img_5_over);
				}, 1000);
				break;
			case '7':
				...
				setTimeout(() => {
					...
					localStorage.setItem(`${id}img7`, img_7_over);
				}, 1000);
				break;
			case '10':
				...
				if (this.state.img10 === img_10_use) {
					buriedPointEvent(activity.use_10);
					this.goRoute();
					return;
				}
				this.chkUser(id, type);
				break;
			case '15':
				...
				if (this.state.img15 === img_15_use) {
					buriedPointEvent(activity.use_15);
					this.goRoute();
					return;
				}
				this.chkUser(id, type);
	...
	chkUser = (id, type) => {
		this.props.$fetch.get(`${API.chkUser}`).then((res1) => {
			if (res1 && res1.msgCode === 'PTM0000') {
				...
				// }
				if (type === '10') {
					buriedPointEvent(activity.ling_10);
					// 调用领券接口
					this.receive(type);
					this.setState({
						img10: res1.data.coupon10 === '0' ? img_10_use : img_10_use
					});
					localStorage.setItem(`${id}img10`, res1.data.coupon10 === '0' ? img_10_use : img_10_use);
				...
				if (type === '15') {
					buriedPointEvent(activity.ling_15);
					// 调用领券接口
					this.receive(type);
					this.setState({
	...
	//去登陆按钮
	goLogin = () => {
		buriedPointEvent(activity.login_click);
		const osType = getDeviceType();
			...
			<div id="huodong">
				...
				{this.state.showAlert && this.state.alertType === 'alert_10' ? (
					<div className={style.alert_bg}>
						...
						<div>
							<img
								src={alert_btn_shiyong}
								onClick={() => {
									buriedPointEvent(activity.yhq_click_10);
									this.goRoute();
								}}
								className={style.alert_btn_shiyong}
							/>
						</div>
					</div>
				) : null}
					...
					<div className={style.alert_bg}>
						...
						<div>
							<img
								src={alert_btn_shiyong}
								onClick={() => {
									buriedPointEvent(activity.yhq_click_15);
									this.goRoute();
								}}
								className={style.alert_btn_shiyong}
							/>
