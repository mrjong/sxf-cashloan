	...
	render() {
		...
		return (
			<div className={styles.freebill}>
				...
				>
					活动规则 <Icon type="right" className={styles.rule_arrow} />
				</div>
					...
					>
						{rewardList.map((item, idx) => (
							<div className={styles.inner} key={idx}>
								<p className={styles.col_scroll_text}>
									{colScrollPhone ? colScrollPhone : '150****1489'}喜获
								</p>
								<div className={styles.square}>
									{' '}
									<i className={styles.free_bg}></i>
									<em className={styles.del_money}>
										{colScrollMoney ? colScrollMoney : '34000,00'}
				...
				{showRowScroll && (
					<div className={styles.row_scroll_wrap}>
						<p className={styles.row_scroll_text}>
							{rewarded ? rewarded : '188****4219'}喜获
							<span className={styles.number_bg}>{rowScrollMoney ? rowScrollMoney : '20000,00'}</span>元免单！
						</p>
					</div>
				)}
